...
fn load_work_into_queue(ip_queue: Arc<Mutex<Vec<Ipv4AddrRange>>>) -> () {
    ...
    for result in rdr.records() {
        ...
        if DEBUG_FLAG { println!("IN QUEUE -> {:?}", record) };
...
fn proc_range(input: Arc<Mutex<Vec<Ipv4AddrRange>>>, output: Arc<Mutex<Vec<ScanResoult>>>, counter: Arc<Mutex<[u64; 2]>>) -> () {
    loop {
...
fn format_record(record: ScanResoult, out: &mut [String; 3]) -> () {
    let octets: [u8; 4] = record.0.octets();
    out[0] = format!("{}.{}.{}.{}", octets[0], octets[1], octets[2], octets[3]);
    out[1] = format!("{}", record.1);
    if BIN_OUTPUT {
        out[2] = encode(&record.2);
    } else {
        out[2] = format!("{}", match std::str::from_utf8(&record.2) { Ok(v)  => v, Err(_) => QUESTIONMARK }.trim_matches(char::from(0)));
    };
}
...
fn main() {
    ...
    let write_queue:    Arc<Mutex<Vec<ScanResoult>>>    = Arc::new(Mutex::new(Vec::new()));
    ...
    loop {
        if write_queue.lock().unwrap().is_empty() {
            ...
            let last: ScanResoult = write_queue.lock().unwrap().pop().unwrap();
            ...
            if w_batch {
                ...
                let c: [u64; 2] = counter.lock().unwrap().clone();
